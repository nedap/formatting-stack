...
(spec/def ::fine-grained-override (spec/map-of (spec/and :formatting-stack.protocols.spec.member/id
                                                         (fn [x]
                                                           (-> x name #{"id"})))
                                               (spec/or :removal  nil?
                                                        :override map?
...
;; a whole substitution
(spec/def ::coarse-grained-override (spec/coll-of any? :kind vector?))

(spec/def ::overrides (spec/or :fine-grained-override   ::fine-grained-override
                               :coarse-grained-override ::coarse-grained-override))

(speced/defn apply-overrides [^::protocols.spec/members members, ^::speced/nilable ^::overrides overrides]
  (cond
    (nil? overrides)
    members,

    (spec/valid? ::coarse-grained-override overrides)
    overrides,
